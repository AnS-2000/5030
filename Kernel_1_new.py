...
def kde_mutual_information(X, Y, bandwidth=1):
    XY = np.vstack([X, Y]).T
...
for i in range(num_columns):
    for j in range(i, num_columns):
        ...
        mutual_info = kde_mutual_information(X, Y, bandwidth=1)

        mutual_info_matrix.iat[i, j] = mutual_info
        mutual_info_matrix.iat[j, i] = mutual_info


is_symmetric = mutual_info_matrix.equals(mutual_info_matrix.T)
print("Is the mutual information matrix symmetric?", is_symmetric)
